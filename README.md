...
- `GET /api/sites` — browse everything in the DB. Supports `?q=` to filter by name, `?status=` for operational status, and `?named_only=true` to exclude unnamed sites. Paginated.
